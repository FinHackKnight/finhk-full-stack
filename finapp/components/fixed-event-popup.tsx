...
export function FixedEventPopup({ event }: FixedEventPopupProps) {
  ...
  return (
    <div
      className={`w-80 rounded-lg border bg-card/95 backdrop-blur-sm shadow-2xl pointer-events-none animate-in fade-in-0 zoom-in-95 duration-200 ${impactColors[event.impactLevel]}`}
      style={{
        position: "fixed",
        top: "1rem",
        right: "1rem",
      ...
      <div className="p-4 space-y-3">
        <div className="space-y-2">
          <div className="flex items-start justify-between gap-2">
            ...
            <span
              className={`px-2 py-1 rounded text-xs font-medium uppercase shrink-0 ${impactBadgeColors[event.impactLevel]}`}
            >
              {event.impactLevel}
            </span>
          </div>
        ...
        <div className="space-y-2">
          <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Affected Markets</p>
            ...
            {event.affectedMarkets.map((market) => (
              <div key={market.symbol} className="flex items-center justify-between text-sm">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span
                    className={`font-mono font-semibold ${market.changePercent > 0 ? "text-emerald-400" : "text-rose-400"}`}
                  >
                    {market.changePercent > 0 ? "+" : ""}
